"""DeepSeekProvider — 通过 DeepSeek API 调用模型（OpenAI 兼容格式）。"""
...
class DeepSeekProvider(BaseProvider):
    """DeepSeek API Provider — OpenAI 兼容格式。

    DeepSeek API 完全兼容 OpenAI Chat Completions API 格式，
    只需更换 endpoint 和 api_key。
    """
    ...
    def __init__(
        ...
        Args:
            ...
            model: 模型名，如 "deepseek-v3" 或 "deepseek-r1"
            api_key: API 密钥
            api_base: 自定义 API 端点，默认 https://api.deepseek.com/v1
            timeout: 默认超时秒数
    ...
    def invoke(
        ...
        """同步调用 DeepSeek Chat API（OpenAI 兼容），返回完整响应。"""
        start = time.time()
    ...
    def stream(
        ...
        """SSE 流式调用 DeepSeek Chat API（OpenAI 兼容）。"""
        messages = self._build_messages(prompt, system_prompt)
